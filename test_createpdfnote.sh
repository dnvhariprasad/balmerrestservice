...
# Usage: ./test_createpdfnote.sh [processInstanceId] [workitemId] [nodownload]
# Pass 'nodownload' as third argument to skip the download step

BASE_URL="${BALMER_API_URL:-http://localhost:8089}"
USERNAME="supervisor"
PASSWORD="Sedin@123456"
PROCESS_INSTANCE_ID="${1:-e-Notes-000000000045-process}"
WORKITEM_ID="${2:-1}"
SKIP_DOWNLOAD="${3:-}"
...
")
    if [ -n "$DOC_INDEX" ]; then
        ...
        # Step 3: Download document with annotations (skip if nodownload)
        if [ "$SKIP_DOWNLOAD" = "nodownload" ]; then
            echo "=============================================="
            echo "Step 3: Skipping download (nodownload flag set)"
            echo "=============================================="
        else
            echo "=============================================="

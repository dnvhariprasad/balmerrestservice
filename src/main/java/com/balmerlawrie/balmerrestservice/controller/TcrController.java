...
@RestController
@RequestMapping("/proc/tcr")
@Tag(name = "Proc TCR", description = "TCR notesheet document operations")
public class TcrController {
    ...
    private static final String TCR_COMMENTS_ATTRIBUTE = "Q_tcr_comments";
    private static final String TCR_ORIGINAL_ATTRIBUTE = "tcr_original";

    private final ObjectMapper mapper = new ObjectMapper();

    @Operation(summary = "Get Original Notesheet", description = "Retrieves the original TCR document (from the '" + TCR_ORIGINAL_ATTRIBUTE + "' work item attribute) from the work item's attachment folder.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<JsonNode> getOriginalNotesheet(
        ...
        JsonNode result = noteSheetService.getOriginalNotesheet(resolvedProcessInstanceId, workitemIdResolved, sessionId, TCR_ORIGINAL_ATTRIBUTE);
    ...
    @Operation(summary = "Get Work Item Comments", description = "Retrieves TCR comments history (from the '" + TCR_COMMENTS_ATTRIBUTE + "' work item attribute) for a work item.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Request processed successfully.", content = @Content(mediaType = "application/json", examples = {
    ...
    public ResponseEntity<JsonNode> getComments(
        ...
        JsonNode result = extractTcrComments(attributesResponse);
        return ResponseEntity.ok(result);
    }

    private JsonNode extractTcrComments(JsonNode attributesResponse) {
        ...
        JsonNode tcrCommentsNode = findFieldIgnoreCase(attributes, TCR_COMMENTS_ATTRIBUTE);
        if (tcrCommentsNode != null) {
            appendCommentsFromNode(tcrCommentsNode, commentsList);
        }
    ...
    @Operation(summary = "Create PDF Note", description = "Creates a PDF from the original TCR document (attribute '" + TCR_ORIGINAL_ATTRIBUTE + "') with appended comments and checks it in against the document referenced by the 'tcr' work item attribute.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<JsonNode> createPdfNote(
        ...
        JsonNode result = noteSheetService.createTcrPdfNote(resolvedProcessInstanceId, workitemIdResolved, sessionId);
        ...
        if (!result.path("success").asBoolean(true)) {
            ...
            if (error.contains("invalid session") || error.contains("401") ||
                ...
                if (sessionId != null) {
                    result = noteSheetService.createTcrPdfNote(resolvedProcessInstanceId, workitemIdResolved, sessionId);
                }
            }
        }

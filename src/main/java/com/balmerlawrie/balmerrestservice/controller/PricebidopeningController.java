...
@RestController
@RequestMapping("/proc/pricebidopening")
@Tag(name = "Proc Pricebidopening", description = "Price Bid Opening notesheet document operations")
public class PricebidopeningController {
    ...
    private static final String PRICEBID_COMMENTS_ATTRIBUTE = "Q_pricebid_comments";
    private static final String PRICEBID_ORIGINAL_ATTRIBUTE = "pricebidopening_original";

    private final ObjectMapper mapper = new ObjectMapper();

    @Operation(summary = "Get Original Notesheet", description = "Retrieves the original Price Bid Opening document (from the '" + PRICEBID_ORIGINAL_ATTRIBUTE + "' work item attribute) from the work item's attachment folder.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<JsonNode> getOriginalNotesheet(
        ...
        JsonNode result = noteSheetService.getOriginalNotesheet(resolvedProcessInstanceId, workitemIdResolved, sessionId, PRICEBID_ORIGINAL_ATTRIBUTE);
    ...
    @Operation(summary = "Get Work Item Comments", description = "Retrieves Price Bid Opening comments history (from the '" + PRICEBID_COMMENTS_ATTRIBUTE + "' work item attribute) for a work item.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Request processed successfully.", content = @Content(mediaType = "application/json", examples = {
    ...
    public ResponseEntity<JsonNode> getComments(
        ...
        if (workitemIdResolved == null || workitemIdResolved.isEmpty() || resolvedProcessInstanceId == null || resolvedProcessInstanceId.isEmpty()) {
            return ResponseEntity.badRequest().body(createError("Missing parameters"));
        }

        JsonNode attributesResponse = workItemAttributesService.fetchWorkItemAttributes(resolvedProcessInstanceId, workitemIdResolved, sessionId);
        JsonNode result = extractPricebidComments(attributesResponse);
        return ResponseEntity.ok(result);
    }

    private JsonNode extractPricebidComments(JsonNode attributesResponse) {
        ...
        JsonNode pricebidCommentsNode = findFieldIgnoreCase(attributes, PRICEBID_COMMENTS_ATTRIBUTE);
        if (pricebidCommentsNode != null) {
            appendCommentsFromNode(pricebidCommentsNode, commentsList);
        }
    ...
    @Operation(summary = "Create PDF Note", description = "Creates a PDF from the original Price Bid Opening document (attribute '" + PRICEBID_ORIGINAL_ATTRIBUTE + "') with appended comments and checks it in against the document referenced by the 'pricebidopening' work item attribute.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<JsonNode> createPdfNote(
        ...
        JsonNode result = noteSheetService.createPricebidopeningPdfNote(resolvedProcessInstanceId, workitemIdResolved, sessionId);
        ...
        if (!result.path("success").asBoolean(true)) {
            ...
            if (error.contains("invalid session") || error.contains("401") ||
                ...
                if (sessionId != null) {
                    result = noteSheetService.createPricebidopeningPdfNote(resolvedProcessInstanceId, workitemIdResolved, sessionId);
                }
            }
        }

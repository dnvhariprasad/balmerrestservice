...
@RestController
@RequestMapping("/proc/corrigendum")
@Tag(name = "Proc Corrigendum", description = "Corrigendum notesheet document operations")
public class CorrigendumController {
    ...
    private static final String CORRIGENDUM_COMMENTS_ATTRIBUTE = "Q_duedatecomments";
    private static final String CORRIGENDUM_ORIGINAL_ATTRIBUTE = "corrigendum_original";

    private final ObjectMapper mapper = new ObjectMapper();

    @Operation(summary = "Get Original Notesheet", description = "Retrieves the original Corrigendum document (from the '" + CORRIGENDUM_ORIGINAL_ATTRIBUTE + "' work item attribute) from the work item's attachment folder.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<JsonNode> getOriginalNotesheet(
        ...
        JsonNode result = noteSheetService.getOriginalNotesheet(resolvedProcessInstanceId, workitemIdResolved, sessionId, CORRIGENDUM_ORIGINAL_ATTRIBUTE);
    ...
    @Operation(summary = "Get Work Item Comments", description = "Retrieves Corrigendum comments history (from the '" + CORRIGENDUM_COMMENTS_ATTRIBUTE + "' work item attribute) for a work item.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Request processed successfully.", content = @Content(mediaType = "application/json", examples = {
    ...
    public ResponseEntity<JsonNode> getComments(
        ...
        JsonNode result = extractCorrigendumComments(attributesResponse);
        return ResponseEntity.ok(result);
    }

    private JsonNode extractCorrigendumComments(JsonNode attributesResponse) {
        ...
        JsonNode corrigendumCommentsNode = findFieldIgnoreCase(attributes, CORRIGENDUM_COMMENTS_ATTRIBUTE);
        if (corrigendumCommentsNode != null) {
            appendCommentsFromNode(corrigendumCommentsNode, commentsList);
        }
    ...
    @Operation(summary = "Create PDF Note", description = "Creates a PDF from the original Corrigendum document (attribute '" + CORRIGENDUM_ORIGINAL_ATTRIBUTE + "') with appended comments and checks it in against the document referenced by the 'corrigendum' work item attribute.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<JsonNode> createPdfNote(
        ...
        JsonNode result = noteSheetService.createCorrigendumPdfNote(resolvedProcessInstanceId, workitemIdResolved, sessionId);
        ...
        if (!result.path("success").asBoolean(true)) {
            ...
            if (error.contains("invalid session") || error.contains("401") ||
                ...
                if (sessionId != null) {
                    result = noteSheetService.createCorrigendumPdfNote(resolvedProcessInstanceId, workitemIdResolved, sessionId);
                }
            }
        }
